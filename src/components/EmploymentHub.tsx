...
const WORKFORCE_HERO = villageConstructionLabor;
...
export default function EmploymentHub({ workers, setWorkers, jobs, setJobs }: EmploymentProps) {
  ...
  // Details and Match Tab states
  const [intelligenceTab, setIntelligenceTab] = useState<'worker' | 'job'>('worker');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="bg-navy-light border border-accent-blue/15 rounded-2xl p-5 space-y-4 shadow-sm">
                  ...
                  <div className="space-y-3 max-h-[440px] overflow-y-auto pr-1">
                    {workers.map((worker, i) => (
                      <div
                        key={worker.id}
                        onClick={() => { setSelectedWorkerId(worker.id); setIntelligenceTab('worker'); }}
                ...
                <div className="bg-navy-light border border-accent-blue/15 rounded-2xl p-5 space-y-4 shadow-sm">
                  ...
                  <div className="space-y-3 max-h-[440px] overflow-y-auto pr-1">
                    {jobs.map((job) => (
                      <div
                        key={job.id}
                        onClick={() => { setSelectedJobId(job.id); setIntelligenceTab('job'); setViewingJob(job); }}
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-950 text-white border border-slate-800 rounded-2xl p-5 shadow-xl">
            ...
            {/* Smart Intelligence Tabs */}
            <div className="grid grid-cols-2 gap-2 mb-4 bg-navy-dark p-1 rounded-xl border border-slate-800">
              <button
                onClick={() => setIntelligenceTab('worker')}
                className={`py-1.5 text-[10px] font-bold rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
                  intelligenceTab === 'worker'
                    ? 'bg-accent-blue text-navy-dark shadow-sm'
              ...
              <button
                onClick={() => setIntelligenceTab('job')}
                className={`py-1.5 text-[10px] font-bold rounded-lg uppercase tracking-wider transition-all cursor-pointer ${
                  intelligenceTab === 'job'
                    ? 'bg-accent-cyan text-navy-dark shadow-sm'
                    : 'text-slate-400 hover:text-slate-300'
                }`}
              >
                Job Fit
            ...
            {intelligenceTab === 'worker' ? (
              currentSelectedWorker ? (
                <div className="space-y-4 text-left">
                  ...
                  <p className="text-[10px] text-slate-500 uppercase font-mono font-bold tracking-wider mb-1">Matched Work Opportunities:</p>
                  <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                    {matchedJobsForWorker.length > 0 ? (
                      matchedJobsForWorker.map((mj) => (
                        <div 
                          key={mj.id} 
                          onClick={() => { setSelectedJobId(mj.id); setIntelligenceTab('job'); setViewingJob(mj); }}
                          className="p-3 rounded-xl bg-navy-dark border border-accent-blue/10 flex flex-col justify-between hover:border-accent-cyan cursor-pointer transition-all"
                        >
              ...
              currentSelectedJob ? (
                <div className="space-y-4 text-left">
                  <div className="p-3 bg-navy-dark border border-accent-cyan/10 rounded-xl">
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-[8px] bg-slate-900 text-accent-cyan border border-accent-cyan/30 px-1.5 py-0.5 rounded font-mono font-bold uppercase">
                        {currentSelectedJob.type}
                      </span>
                      <span className="text-[9px] font-mono text-slate-400">{currentSelectedJob.id}</span>
                    </div>
                    <h4 className="font-bold text-xs text-white mt-1">{currentSelectedJob.title}</h4>
                    <p className="text-[10px] text-slate-400 mt-0.5">{currentSelectedJob.employer}</p>
                    <p className="text-[9px] text-accent-cyan/85 font-mono mt-1">Wage: {currentSelectedJob.wage}</p>
                  </div>

                  <p className="text-[10px] text-slate-500 uppercase font-mono font-bold tracking-wider mb-1">Best-Fit Candidate Matches:</p>
                  <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                    {(() => {
                      const matchedWorkers = workers.map(w => ({
                        ...w,
                        score: getMatchScore(w, currentSelectedJob)
                      }))
                      .filter(w => w.score > 0)
                      .sort((a, b) => b.score - a.score);

                      if (matchedWorkers.length > 0) {
                        return matchedWorkers.map((mw) => (
                          <div 
                            key={mw.id} 
                            onClick={() => { setSelectedWorkerId(mw.id); setIntelligenceTab('worker'); }}
                            className="p-3 rounded-xl bg-navy-dark border border-accent-blue/10 flex flex-col justify-between hover:border-accent-blue cursor-pointer transition-all"
                          >
                            <div className="flex justify-between items-start">
                              <div>
                                <span className="text-[8px] bg-emerald-950 text-emerald-400 px-1.5 py-0.5 rounded font-mono font-bold">
                                ...
                                <h5 className="font-bold text-xs text-white mt-1.5">{mw.name}</h5>
                                <p className="text-[9px] text-slate-400">{mw.skills.join(', ')}</p>
                              </div>
                              <span className="text-[9px] text-slate-400">{mw.age} Yrs</span>
                            </div>

                            {/* Match Progress circle */}
                            <div className="w-full bg-navy-light h-1 rounded-full overflow-hidden mt-3">
                              <div className="bg-accent-blue h-full" style={{ width: `${mw.score}%` }} />
                            </div>
                          </div>
                        ));
                      } else {
                        return (
                          <div className="text-center py-6 text-xs text-slate-500">
                            No local workers currently match this job's requirements.
                          </div>
                        );
                      }
                    })()}
                  ...
                  <div className="pt-3 border-t border-slate-850 text-center flex flex-col gap-2">
                    <button
                      onClick={() => setViewingJob(currentSelectedJob)}
                      className="w-full py-1.5 bg-accent-cyan hover:bg-accent-blue text-navy-dark hover:text-white transition-all text-[11px] font-bold rounded-lg cursor-pointer"
                    >
                      View Detailed Job Spec
                    </button>
                    <span className="text-[9px] text-slate-500 italic">
                      Contact GVMC Zonal Office to refer workers.
                    </span>
                  </div>
                </div>
              ) : (
                <div className="text-center py-8 text-slate-500 text-xs">
                  Select a job posting to view its smart matches.
                </div>
              )
            )}
          </div>
        ...
        {viewingJob && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-dark/85 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-navy-light border border-accent-blue/30 rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl animate-fade-in"
            >
              {/* Header */}
              <div className="flex justify-between items-center p-5 border-b border-navy-dark">
                <div className="flex items-center gap-2">
                  <Briefcase className="w-5 h-5 text-accent-cyan" />
                  <div>
                    <h3 className="font-bold text-sm text-white font-display">Opportunity Spec & Match Analysis</h3>
                    <p className="text-[10px] text-slate-400 font-mono text-left">ID: {viewingJob.id}</p>
                  </div>
                </div>
                <button 
              ...
              {/* Body */}
              <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto text-left">
                {/* Main Details Card */}
                <div className="bg-navy-dark border border-accent-blue/10 p-4 rounded-xl space-y-3">
                  <div className="flex justify-between items-start">
                    <div className="text-left">
                      <span className="text-[8px] bg-accent-cyan/15 text-accent-cyan border border-accent-cyan/30 px-2 py-0.5 rounded font-mono font-bold uppercase">
                        {viewingJob.type}
                      </span>
                      <h4 className="font-bold text-base text-white mt-1.5 leading-tight">{viewingJob.title}</h4>
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-lg font-extrabold text-accent-cyan">{viewingJob.wage}</span>
                      <p className="text-[9px] text-slate-500">Proposed Compensation</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4 pt-3 border-t border-navy-light/40 text-xs">
                    <div className="text-left">
                      <p className="text-slate-400 text-[10px] font-mono">LOCATION WARD</p>
                      <p className="font-semibold text-white mt-0.5 flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5 text-accent-blue" />
                        {viewingJob.ward.split(' (')[0]}
                      </p>
                    </div>
                    <div className="text-left">
                      <p className="text-slate-400 text-[10px] font-mono">STATUS</p>
                      <p className="font-semibold text-emerald-400 mt-0.5 flex items-center gap-1">
                        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        Active ({viewingJob.status})
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 text-left">
                  <h5 className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider">Required Skillsets</h5>
                  <div className="flex flex-wrap gap-1.5">
                    {viewingJob.skillsRequired.map((s, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded-lg text-xs bg-navy-dark text-white border border-accent-blue/15 font-medium">
                        {s}
                      </span>
                    ))}
                  </div>
                ...
                {/* Qualified Local Workers list */}
                <div className="space-y-2 text-left">
                  <h5 className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider">Qualified Local Workers</h5>
                  <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                    {(() => {
                      const matched = workers.map(w => ({
                        ...w,
                        score: getMatchScore(w, viewingJob)
                      }))
                      .filter(w => w.score > 0)
                      .sort((a, b) => b.score - a.score);

                      if (matched.length > 0) {
                        return matched.map((mw, i) => (
                          <div key={mw.id} className="p-3 bg-navy-dark/60 rounded-xl border border-accent-blue/10 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <img 
                                src={getAvatarUrl(mw.id, i)} 
                                alt={mw.name} 
                                referrerPolicy="no-referrer"
                                className="w-8 h-8 rounded-full object-cover border border-accent-blue/30 shrink-0"
                              />
                              <div className="text-left">
                                <p className="font-bold text-xs text-white">{mw.name}</p>
                                <p className="text-[10px] text-slate-400">{mw.skills.join(', ')}</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <span className="text-[9px] bg-emerald-950/80 text-emerald-400 px-2 py-0.5 rounded-full font-mono font-bold border border-emerald-900/30">
                                {mw.score}% Fit
                              </span>
                              <p className="text-[10px] text-accent-blue font-medium mt-1">{mw.phone}</p>
                            </div>
                          </div>
                        ));
                      } else {
                        return (
                          <div className="text-center py-4 bg-navy-dark/40 rounded-xl text-slate-500 text-xs">
                            No local workers registered for these skills yet.
                          </div>
                        );
                      }
                    })()}
              ...
              {/* Footer */}
              <div className="p-4 bg-navy-dark border-t border-navy-light/40 flex gap-3">
                <button
                  onClick={() => setViewingJob(null)}
                  className="flex-1 py-2 rounded-xl text-xs font-bold text-slate-300 bg-navy-light hover:bg-navy-dark transition-all border border-accent-blue/10 cursor-pointer"
                >
                  Close Spec
                </button>
                <button
                  onClick={() => {
                    alert(`Referral inquiry for Job ID ${viewingJob.id} has been submitted to Gajuwaka Zonal GVMC Desk!`);
                    setViewingJob(null);
                  }}
                  className="flex-1 py-2 rounded-xl text-xs font-bold text-navy-dark bg-accent-cyan hover:bg-accent-blue hover:text-white transition-all cursor-pointer shadow-md"
                >
                  Inquire / Refer Candidate
                </button>
              </div>
            </motion.div>
          </div>
